...
import static com.task11.Util.badRequest;
import static com.task11.Util.ok;
import static com.task11.Validator.*;
...
public class ApiHandler implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
		...
		switch (path) {
			case "/signup":
				if (List.of("POST").contains(requestEvent.getHttpMethod())) {
					return addHeaders(handleSignup(requestEvent));
				}
			case "/signin":
				if (List.of("POST").contains(requestEvent.getHttpMethod())) {
					return addHeaders(handleSignin(requestEvent));
				}
			case "/tables":
				if (List.of("POST", "GET").contains(requestEvent.getHttpMethod())) {
					return addHeaders(handleTables(requestEvent));
				}
			case "/reservations":
				if (List.of("POST", "GET").contains(requestEvent.getHttpMethod())) {
					return addHeaders(handleReservations(requestEvent));
				}
			default:
				if (path.startsWith("/tables/") && List.of("GET").contains(requestEvent.getHttpMethod())) {
					return addHeaders(handleTablesById(requestEvent));
				}
		}
		return addHeaders(new APIGatewayProxyResponseEvent().withStatusCode(HttpStatus.SC_NOT_FOUND));
	}
		...
		try {
			...
			AdminCreateUserRequest adminCreateUserRequest = new AdminCreateUserRequest()
					...
					.withUserAttributes(
							new AttributeType()
									.withName("email")
									.withValue(requestMap.get("email")),
							new AttributeType()
	...
	private String getUserPoolId() {
		return cognitoClient.listUserPools(new ListUserPoolsRequest().withMaxResults(60))
				.getUserPools().stream()
	...
	private APIGatewayProxyResponseEvent addHeaders(APIGatewayProxyResponseEvent response) {
		return response.withHeaders(Map.of(
